...
│  (hand only)    │ │  (wrist only)   │ │  (hand + wrist)          │
└────────┬────────┘ └────────┬────────┘ └────────────┬────────────┘
         │                   │                       │
         ▼                   ▼                       ▼
┌────────────────────────────────────────────────────────────────┐
...
│  │ (keypoints)  │  │ /right_wrist │  │     world->*_wrist │   │
│  └──────┬───────┘  └──────┬───────┘  └─────────┬──────────┘   │
└─────────┼─────────────────┼────────────────────┼───────────────┘
          │                 │                    │
          ▼                 ▼                    ▼
...
│ (retargeting)   │ │  (dynamic TF)   │ │  chest->*_chest         │
└────────┬────────┘ └────────┬────────┘ │  *_wrist->tianji_*      │
         │                   │          └────────────┬────────────┘
         ▼                   └───────────────────────┤
┌─────────────────┐                                  ▼
...
world
├── head
├── chest (computed from head, -0.3 m in Z)
│   ├── left_chest (static: Rx=-90deg, Rz=90deg, Tz=37 mm)
│   └── right_chest (static: Rx=90deg, Rz=90deg, Tz=37 mm)
├── left_wrist
│   └── tianji_left (static: Rz=180deg Ry=-90deg Rz=225deg)
├── right_wrist
│   └── tianji_right (static: Ry=270deg Rz=225deg)
├── left_arm (upper-arm tracker, for arm angle following)
└── right_arm (upper-arm tracker, for arm angle following)
```
