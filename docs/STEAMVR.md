...
HTC Vive Tracker for arm pose tracking + Wuji Glove (default; MANUS supported via `wujihand_ik.yaml::input_source`), arm pose tracking via SteamVR.
...
|---------------|-------------|
| `ros2 launch wuji_teleop_bringup wuji_teleop.launch.py` | Hand + arm + camera (recommended) |
| `ros2 launch wuji_teleop_bringup wuji_teleop.launch.py enable_camera:=false` | Hand + arm (no camera) |

### Parameter Description

The hand input source (`wuji_glove` / `manus`) is **not** a launch arg — it is read from
`src/output_devices/wujihand_output/config/wujihand_ik.yaml::input_source`. The SteamVR guide
keeps `arm_input:=tracker` (the launch default).
...
|-----------|---------|-------------|
| `arm_input` | `tracker` | Arm input source: `tracker` (HTC Vive) or `pico` |
| `enable_camera` | `true` | Whether to start cameras |
| `enable_rviz` | `false` | Whether to start RViz visualization |

> `enable_head` is a `camera_launch.py` parameter only (see `src/camera/launch/camera_launch.py`); the top-level `wuji_teleop.launch.py` toggles all cameras together via `enable_camera`.
...
|--------|-------------|--------------------------|
| Hand only | Wuji Glove → Wuji Hand (MANUS optional via yaml) | `wuji_teleop_hand.launch.py` |
| Arm only | Vive Tracker → Tianji Arm | `wuji_teleop.launch.py enable_hand:=false` |
| Hand + Arm | Wuji Glove + Tracker → Full teleoperation | `wuji_teleop.launch.py` |
| Hand + Arm + Camera | Full teleoperation + cameras | `wuji_teleop.launch.py` |
| Single side (Left/Right) | Single arm + single hand debug | `wuji_teleop.launch.py` |
