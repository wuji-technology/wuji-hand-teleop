...
void OnPXREAClientCallback(void* context, PXREAClientCallbackType type, int status, void* userData)
{
    ...
    case PXREADeviceStateJson:
        ...
        try {
            ...
            // wuji-hand-teleop patch (2026-06-01): stringToPoseArray / stringToVelocityArray
            // call std::stod, which throws std::invalid_argument on malformed input.
            // Without this catch the exception escapes the callback thread.
            std::cerr << "Pose/velocity parse error (invalid_argument): " << e.what() << std::endl;
        } catch (const std::out_of_range& e) {
            // wuji-hand-teleop patch (2026-06-01): std::stod throws std::out_of_range when
            // the parsed value is too large; same callback-thread escape concern.
            std::cerr << "Pose/velocity parse error (out_of_range): " << e.what() << std::endl;
        }
            break;
    }
}
...
// wuji-hand-teleop patch (2026-06-01): upstream returned int but LeftHandScale is
// already double (parsed via leftHand["scale"].get<double>()), so the int return
// type silently truncated the value. Widened to double to preserve precision.
double getLeftHandScale() {
    std::lock_guard<std::mutex> lock(leftHandMutex);
...
// wuji-hand-teleop patch (2026-06-01): same int -> double widening as getLeftHandScale.
double getRightHandScale() {
    std::lock_guard<std::mutex> lock(rightHandMutex);
...
PYBIND11_MODULE(xrobotoolkit_sdk, m) {
    ...
    // wuji-hand-teleop patch (2026-06-01): expose hand scale getters that upstream
    // forgot to register. These return the per-hand calibration scale parsed from
    // the SDK callback's `leftHand["scale"]` / `rightHand["scale"]` JSON fields.
    m.def("get_left_hand_scale", &getLeftHandScale, "Get the left hand calibration scale.");
    m.def("get_right_hand_scale", &getRightHandScale, "Get the right hand calibration scale.");
