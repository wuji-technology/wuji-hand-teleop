...
class DeviceStatusWidget(QGroupBox):
    ...
    def _setup_ui(self):
        ...
        self.info_label.setTextFormat(Qt.RichText)  # 启用富文本/HTML显示
...
class CameraStatusWidget(QGroupBox):
    ...
    def _setup_ui(self):
        ...
        self.cam_head = TopicStatusWidget("头部")
        self.cam_left = TopicStatusWidget("左腕")
        self.cam_right = TopicStatusWidget("右腕")
...
class StereoHeadStatusWidget(QGroupBox):
    """Widget to display stereo head camera (头部双目相机) status."""

    def __init__(self, parent=None):
        super().__init__("头部双目相机", parent)
        self._setup_ui()
    ...
    def _setup_ui(self):
        ...
        # 左眼话题
        self.topic_left = TopicStatusWidget("左眼")
        topic_layout.addWidget(self.topic_left)

        # 右眼话题
        self.topic_right = TopicStatusWidget("右眼")
    ...
    def set_camera_available(self, available: bool, info: str = ""):
        ...
        if available:
            self.camera_label.setText(f"● 相机 {info}")
            self.camera_label.setStyleSheet("color: #00aa00;")
        else:
            self.camera_label.setText("○ 相机未连接")
            self.camera_label.setStyleSheet("color: #cc0000;")
