...
# Camera topic names
# D405 只发布 image_rect_raw (无独立 RGB 模块), D435 两者都发布, 用 rect_raw 兼容两者
CAMERA_TOPICS = {
...
class TeleopMonitorNode(Node):
    ...
    def __init__(self):
        ...
        # Stereo head camera state (头部双目相机)
        ...
        self.stereo_head_camera_available = False  # 双目相机硬件是否可用
        self.stereo_head_loopback_available = False  # v4l2loopback 是否可用
    ...
    def get_hand_input_info(self) -> str:
        ...
        if self.hand_input_data_size == 126:
            ...
            return f"{self.hand_input_data_size}值"
        return "--"
